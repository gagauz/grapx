...
public class BSpline {
    ...
    public float getValue(int x) {
        ...
        return a[index]
                + b[index] * (x - xx[index])
                + c[index] * ((float) Math.pow(x - xx[index], 2))
                + d[index] * ((float) Math.pow(x - xx[index], 3));
    }
     ...
     * @return the interpolated value
     */
    public float getFastValue(int x) {
        // Fast check to see if previous index is still valid
        if (storageIndex > -1 && storageIndex < xx.length - 1 && x > xx[storageIndex]
                && x < xx[storageIndex + 1]) {
    ...
    public float getDx(int x) {
        ...
        return b[index]
                ...
                + 3 * d[index] * ((float) Math.pow(x - xx[index], 2));
    }
